...
nocite: |
    @lambrecht1996, @janko2001, @vilkuna1989, @benko2014, @king1995
---
...
adverbials %>% 
    mutate(location=gsub("S","L",location)) %>% 
    count(lang,location) %>% 
    group_by(lang) %>% 
    mutate(freq=100*(n/sum(n))) %>% 
    ggplot(aes(x=location,y=freq,fill=lang)) +
        geom_bar(stat="identity", position=position_dodge(width=0.2), width=0.5) + 
        facet_grid(~lang) + 
        geom_text_repel(aes(label=paste0("",n)),color="white",vjust=1.3) +
...
```{r indandwithout, echo=FALSE, fig.cap='Distributions of the three locations in the whole data set and in the data including all the indicators for ICx'}

library(ggplot2)


if(!exists("modeldata")){
    modeldata <- left_join(adverbials,headverbs,by=c("headverb","lang"))  %>% 
        mutate(is_presentational = ifelse(is.na(is_presentational),"n",is_presentational))
    modeldata <- modeldata %>%
            mutate(all_indicators=
                   case_when(is_presentational == "y" & 
                         corpustype =="press" &
                         group == "Y" &
                         subjtype == "long"  ~ "y",
                       TRUE ~ "n"))
...
GetTab <- function(input, ind_type){
    input  %>% 
        xtabs(~lang + location3,.) %>%  
        prop.table(.,1) %>%
        as_tibble %>% 
        mutate(n=100*n) %>% 
        left_join(.,
                  input %>% count(lang,location3),
                  by=c("lang","location3")
                  )  %>% 
        mutate(`indicators`=ind_type) %>% 
        return
}


rbind(
    modeldata %>% GetTab(.,"All the cases")  ,
    modeldata  %>% 
        filter(all_indicators=="y") %>% 
        GetTab(.,"Just the ones with all the indicators")
    ) %>% 
    ggplot(., aes(x=location3,y=n,fill=location3)) + 
        geom_bar(stat="identity") + 
        geom_text(aes(label=paste0("n=",nn))) + 
        facet_grid(lang ~ indicators)  +
         ylab("Percent") + xlab("Location")
